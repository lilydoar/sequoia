...
/*
 * MACRO EXPLANATION:
 * Example usage of the state type macros:
 *
 * // Define a state for a weather simulation:
 * DECLARE_STATE_TYPE(WeatherState,
 *     double temperature;
 *     double humidity;
 *     int is_raining;
 * );
 *
 * // This expands to create two types:
 * // 1. WeatherState - Contains just the data
 * // 2. WeatherState_State - Full state node with transitions
 *
 * // The transition type is automatically created:
 * // WeatherState_Transition - Typed transitions for this state
 *
 * // Example usage:
 * WeatherState sunny = { .temperature = 25.0, .humidity = 0.3, .is_raining = 0
 * }; WeatherState rainy = { .temperature = 18.0, .humidity = 0.8, .is_raining =
 * 1 };
 *
 * // Add states to model
 * State* sunny_state = add_state(model, &sunny, sizeof(WeatherState), "sunny");
 * State* rainy_state = add_state(model, &rainy, sizeof(WeatherState), "rainy");
 *
 * // Add transitions
 * add_transition(sunny_state, rainy_state, 0.3);  // 30% chance of rain
 * add_transition(rainy_state, sunny_state, 0.4);  // 40% chance of clearing
 */

/* Create a custom state type with the given name and data fields
 * name: Name prefix for the generated types
 * data_fields: Structure fields for the state's data
 *
 * Creates two types:
 * - name: A struct containing just the data fields
 * - name_State: A full state node containing the data and transition info
 */
 ...
 * state_type: Name of the state type to create transitions for
 *
 * Creates:
 * - state_type_Transition: A struct for transitions between states of this type
 */
...
State *add_state(Model *model, const void *data, size_t data_size,
                 const char *name) {
  if (!model || !data || !name)
    return NULL;
...
/* Sets the initial state of the model without triggering callbacks
 * model: The Markov model
 * state: State to set as initial
 */
...
/* Moves the model to the specified state, triggering transition callbacks
 * model: The Markov model
...
State **find_states_by_tag(const Model *model, TagFilter filter, void *context,
                           size_t *count) {
...
void set_transition_callback(Model *model,
 ...
 */
State *next_state(const Model *model, const State *current) {
  if (!model || !current)
    return NULL;
 ...
 */
State *find_matching_state(const Model *model, const void *external_data,
 ...
 */
json_object *serialize_model(const Model *model,
  ...
  /*    if (tags_array) {*/
  /*      for (StateTag *tag = model->states[i].tags; tag; tag = tag->next) {*/
  /*        json_object_array_add(tags_array,
   * json_object_new_string(tag->tag));*/
  /*      }*/
 ...
 * }
 */
MarkovError deserialize_model(Model *model, json_object *root,
  ...
  /*    json_object *trans_obj = json_object_array_get_idx(transitions_array,
   * j);*/
